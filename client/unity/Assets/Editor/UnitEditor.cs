...
public static class UnitGizmoDrawer
{
    ...
    public static void OnDrawGizmo(Unit unit, GizmoType type)
    {
        ...
        var targetStr = Unit.IsNullOrDead(unit._target) ? unit._target == null? "null": $"dead({unit._target.unitID})" : unit._target.unitID.ToString();
        ...
        Handles.Label(unit.transform.position, $"{unit.unitID},{unit.hp},{unit._currentState},{targetStr},{destStr}");
    }
}