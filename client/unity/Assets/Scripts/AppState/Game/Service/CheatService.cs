...
namespace Game.Service
{
    ...
    {
        static public string ParseMessage(string msg, GameState gameState)
        {
            ...
            if (paramListToCall.Count > 0)
                methodInfo.Invoke(gameState, paramListToCall.ToArray());
            else
                methodInfo.Invoke(gameState, (object[])null);
        ...
        static public List<string> GetCommandList()
        {
            ...
            var methods = typeof(GameState).GetMethods().Where(x =>
            {
